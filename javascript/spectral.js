...
function compareHex(hex) {
	
	
	var hexList = populateHexList();
	...
	for (var i = 1; i < hexList.length; i+=2) {
		var h_rgbint = parseInt(hexList[i], 16);
		var h_r = (h_rgbint >> 16 ) & 255;
		var h_g = (h_rgbint >> 8 ) & 255;
		var h_b = (h_rgbint) & 255;
		
		// get the absolute difference 
		var dif = Math.abs(h_r - r) + Math.abs(h_g - g) + Math.abs(h_b - b);
		// set the closest color of the current color if it has the smallest difference
		if (dif < closestHex_val) {
			closestHex_val = dif;
			closestHex = hexList[i];
			closestHex_name = hexList[i-1];
		}
	}
	
	document.getElementById("title").innerHTML = closestHex_name;
	return closestHex;
}

function populateHexList(hexList) {
...
//readTextFile("file:///C:/Users/G369/Documents/rgb.txt");

var list= `cloudy blue	#acc2d9	
...
function populateHexList() {
	
	// remove tabs and new lines from list
	list = list.replace(/\t/g, ' ').replace(/[\r\n]+/g, '#');;
	var hexList = list.split('#');
	
	// remove extra spaces
	for (var i = 0; i < hexList.length; i++) {
		if (hexList[i].charAt(hexList[i].length - 1) == ' ') {
			hexList[i] = hexList[i].slice(0, -1);
		}
	}
...
var test_hex = (23 | (90 << 8) | (20 << 16)).toString(16);
document.getElementById("title").style.backgroundColor = "#" + test_hex;
console.log(compareHex(test_hex));
document.body.style.backgroundColor = "#" + compareHex(test_hex);
//console.log(list);